...
def update_transcription(recording_id: str, transcription: str):
    ...
    response = requests.put(url=f"{host}/recordings/{recording_id}/transcription", data=json.dumps(request_payload))
    print(response.json())
...
def update_prompts(recording_id: str, prompts: list[str]):
    ...
    response = requests.put(url=f"{host}/recordings/{recording_id}/prompts", data=json.dumps(request_payload))
    print(response.json())
...
def create_image_generation(recording_id: str, image_generation: ImageGenerationCreate):
    response = requests.post(url=f"{host}/recordings/{recording_id}/image-generations/", data=image_generation.model_dump_json())
    print(response.json())
...
def update_image_generation(recording_id: str, generation_id: str, image_generation: ImageGenerationUpdate):
    response = requests.put(url=f"{host}/recordings/{recording_id}/image-generations/{generation_id}", data=image_generation.model_dump_json())
    print(response.json())
